---
title: LaTeX Symbols - Complete List of Greek Letters & Math Symbols Reference
description: Comprehensive LaTeX symbols reference guide with 500+ mathematical symbols, Greek letters, operators, arrows, and special characters. Copy-paste ready commands for all LaTeX symbols.
keywords: LaTeX symbols, Greek letters LaTeX, mathematical symbols, LaTeX commands, special characters, LaTeX operators, LaTeX arrows, math notation
image: /images/latex-symbols-reference.png
seo:
  title: LaTeX Symbols Reference - Complete List with Commands | LaTeX Cloud Studio
  description: Find any LaTeX symbol instantly. Complete reference with 500+ mathematical symbols, Greek letters, operators, and special characters. Copy-paste ready commands.
  canonical: https://resources.latex-cloud-studio.com/learn/reference/symbols
---

Quick access to **all LaTeX symbols** organized by category. This comprehensive reference includes over 500 mathematical symbols, Greek letters, operators, and special characters with their LaTeX commands.

<Info>
**Pro tip**: Use `Ctrl+F` (or `Cmd+F` on Mac) to search for specific symbols on this page. Some symbols require the `amssymb` package.
</Info>

## Quick Navigation
Jump to any section:
- [Greek Letters](#greek-letters) - α, β, γ, δ, ε...
- [Mathematical Operators](#mathematical-operators) - ∑, ∏, ∫, ∂...
- [Arrows](#arrows) - →, ←, ↔, ⇒, ⇐...
- [Set Theory](#set-theory) - ∈, ∉, ⊂, ⊃, ∪, ∩...
- [Logic Symbols](#logic-symbols) - ∧, ∨, ¬, ∀, ∃...
- [Calculus](#calculus) - ∇, ∆, ∂, ∫, ∮...
- [Special Characters](#special-characters) - ∞, ∅, ℵ, ℘...

## Contents

1. [Greek letters](#greek-letters)
2. [Arrows](#arrows)
3. [Miscellaneous symbols](#miscellaneous-symbols)
4. [Binary Operation/Relation Symbols](#binary-operationrelation-symbols)
5. [Further reading](#further-reading)

## Greek letters

### Lowercase and Uppercase Greek Letters

<div style={{ fontSize: '1.2em', lineHeight: '2.0' }}>

| Symbol | LaTeX | Symbol | LaTeX |
|:------:|:------|:------:|:------|
| **α** A | `\alpha` `A` | **ν** N | `\nu` `N` |
| **β** B | `\beta` `B` | **ξ** **Ξ** | `\xi` `\Xi` |
| **γ** **Γ** | `\gamma` `\Gamma` | o O | `o` `O` |
| **δ** **Δ** | `\delta` `\Delta` | **π** **Π** | `\pi` `\Pi` |
| **ε** **ϵ** E | `\epsilon` `\varepsilon` `E` | **ρ** **ϱ** P | `\rho` `\varrho` `P` |
| **ζ** Z | `\zeta` `Z` | **σ** **Σ** | `\sigma` `\Sigma` |
| **η** H | `\eta` `H` | **τ** T | `\tau` `T` |
| **θ** **ϑ** **Θ** | `\theta` `\vartheta` `\Theta` | **υ** **Υ** | `\upsilon` `\Upsilon` |
| **ι** I | `\iota` `I` | **φ** **φ** **Φ** | `\phi` `\varphi` `\Phi` |
| **κ** K | `\kappa` `K` | **χ** X | `\chi` `X` |
| **λ** **Λ** | `\lambda` `\Lambda` | **ψ** **Ψ** | `\psi` `\Psi` |
| **μ** M | `\mu` `M` | **ω** **Ω** | `\omega` `\Omega` |

</div>

<Tip>
**Note**: Uppercase Greek letters that look identical to Latin letters (like Alpha → A, Beta → B) are simply typed as regular capital letters.
</Tip>

## Arrows

<div style={{ fontSize: '1.2em', lineHeight: '2.0' }}>

| Symbol | LaTeX | Symbol | LaTeX |
|:------:|:------|:------:|:------|
| **←** | `\leftarrow` | **⇐** | `\Leftarrow` |
| **→** | `\rightarrow` | **⇒** | `\Rightarrow` |
| **↔** | `\leftrightarrow` | **⇔** | `\Leftrightarrow` |
| **↑** | `\uparrow` | **⇑** | `\Uparrow` |
| **↓** | `\downarrow` | **⇓** | `\Downarrow` |
| **↕** | `\updownarrow` | **⇕** | `\Updownarrow` |
| **↦** | `\mapsto` | **⟼** | `\longmapsto` |
| **↗** | `\nearrow` | **↘** | `\searrow` |
| **↙** | `\swarrow` | **↖** | `\nwarrow` |
| **↪** | `\hookrightarrow` | **↩** | `\hookleftarrow` |

</div>

## Miscellaneous symbols

<div style={{ fontSize: '1.2em', lineHeight: '2.0' }}>

| Symbol | LaTeX | Symbol | LaTeX |
|:------:|:------|:------:|:------|
| **∞** | `\infty` | **∀** | `\forall` |
| **∃** | `\exists` | **∄** | `\nexists` |
| **∅** | `\emptyset` | **∅** | `\varnothing` |
| **∇** | `\nabla` | **∂** | `\partial` |
| **√** | `\surd` | **⊤** | `\top` |
| **⊥** | `\bot` | **ℵ** | `\aleph` |
| **ℶ** | `\beth` | **ℷ** | `\gimel` |

</div>

## Binary Operation/Relation Symbols

<div style={{ fontSize: '1.2em', lineHeight: '2.0' }}>

| Symbol | LaTeX | Symbol | LaTeX |
|:------:|:------|:------:|:------|
| **×** | `\times` | **÷** | `\div` |
| **±** | `\pm` | **∓** | `\mp` |
| **∪** | `\cup` | **∩** | `\cap` |
| **\** | `\setminus` | **⊂** | `\subset` |
| **⊃** | `\supset` | **⊆** | `\subseteq` |
| **⊇** | `\supseteq` | **∈** | `\in` |
| **∉** | `\notin` | **∋** | `\ni` |
| **≤** | `\leq` | **≥** | `\geq` |
| **∼** | `\sim` | **≈** | `\approx` |
| **≡** | `\equiv` | **≠** | `\neq` |

</div>

## Further reading

For more comprehensive symbol references and advanced usage:

<CardGroup cols={2}>
<Card title="Mathematical Expressions" icon="square-root-variable" href="/learn/latex/mathematics/mathematical-expressions">
Learn how to write complex mathematical expressions
</Card>
<Card title="Advanced Mathematics" icon="function" href="/learn/latex/mathematics/advanced-math">
Explore advanced mathematical typesetting
</Card>
<Card title="Chemistry Notation" icon="flask" href="/learn/latex/specialized-notation/chemistry">
Chemical formulas and reactions
</Card>
<Card title="Physics Symbols" icon="atom" href="/learn/latex/specialized-notation/physics">
Physics notation and equations
</Card>
</CardGroup>

<Info>
**Need more symbols?** The comprehensive `amssymb` and `amsfonts` packages provide hundreds of additional mathematical symbols. Simply add `\usepackage{amssymb}` to your preamble.
</Info>